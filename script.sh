...
pacstrap -C pacman.conf target_root base base-devel lightdm lightdm-gtk-greeter networkmanager xdg-user-dirs nitrogen linux-raspberrypi4-5.4.y linux-raspberrypi4-5.4.y-headers raspberrypi-firmware mesa-arm-git mesa-vdpau-arm-git rofi-git lxsession dunst alsa-utils raspberrypi-bootloader xorg-xinit xorg-server
